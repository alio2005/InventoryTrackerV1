...
export default function ClosedBookingsPage() {
  ...
  const getStatusClasses = (status: string) => {
    ...
    if (status === "cancelled") return "bg-zinc-900 text-zinc-300";
    ...
    <main className="min-h-screen bg-black px-4 py-8 text-zinc-100 dark:bg-black dark:text-zinc-100">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 flex flex-col gap-4 rounded-3xl border border-zinc-800 bg-zinc-950 p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between dark:border-zinc-800 dark:bg-zinc-950">
          <div>
            <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Inventory System</p>
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-3">
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950 p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Returned</p>
            <p className="mt-3 text-3xl font-bold tracking-tight text-emerald-600">{returnedCount}</p>
          </div>
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950 p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Cancelled</p>
            <p className="mt-3 text-3xl font-bold tracking-tight text-zinc-400">{cancelledCount}</p>
          </div>
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950 p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Declined</p>
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row">
          <input
            ...
            className="w-full rounded-2xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-sm outline-none transition focus:border-zinc-400 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
          />
          <select
            value={statusFilter}
        ...
        <div className="overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          ...
          {loading ? (
            <div className="px-6 py-8 text-sm text-zinc-500 dark:text-zinc-400">Loading...</div>
          ) : filtered.length === 0 ? (
            <div className="px-6 py-8 text-sm text-zinc-500 dark:text-zinc-400">No closed bookings found.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full border-collapse">
                <thead>
                  <tr className="border-b border-zinc-800 dark:border-zinc-800">
                    {["Item", "Unit", "Borrower", "Qty", "Borrowed", "Returned", "Status", "Notes", "Closed On"].map((h) => (
                      <th key={h} className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filtered.map((b) => (
                    <tr key={b.id} className="border-b border-zinc-800 last:border-b-0 dark:border-zinc-800">
                      <td className="px-6 py-4 text-sm font-medium">{getItemName(b)}</td>
                      <td className="px-6 py-4 text-sm text-zinc-500 dark:text-zinc-400">{getUnitCode(b) ?? "—"}</td>
                      <td className="px-6 py-4 text-sm">
                        <div className="font-medium">{b.borrower_name}</div>
                        {b.borrower_email && <div className="text-xs text-zinc-500">{b.borrower_email}</div>}
                      </td>
                      ...
                      <td className="px-6 py-4 text-sm text-zinc-500 dark:text-zinc-400">{b.notes ?? "—"}</td>
